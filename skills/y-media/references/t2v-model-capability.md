...
**症状**: 15s 内写 6 个 Shot,模型只生成 1-2 个景别,中间全塌缩。

**根因**: t2v 模型的"注意力窗口"有限,15s 内处理不了 6 次景别切换。镜头越多,每镜分到的生成预算越少,信息熵过高,模型选择"最安全的 1-2 个景别"覆盖全程。
...
**铁律**: 15s 内 ≤3 镜,单镜 ≥4s,每个时间段用时间锚点标注。
...
| 方式 | 强度 | 写法 | 适用 |
| --- | --- | --- | --- |
| 精确时间锚点 | 最强 | `0.0-5.0s: ...` / `5.0-11.0s: ...` | 10-15s,3-4镜 |
