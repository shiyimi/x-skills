...
Lock 5 style items across the whole segment: 字体族 (one family — see anti-mojibake rule below), 色板 (≤2 colors; 数据卖点常金黄加粗; 金句黑底白字+关键词黄底高亮), 位置 (per type), 动画 (1-2 kinds), 描边/阴影 (uniform). Design motion in three phases — 入场 (0.2-0.4s: 弹入放大/上滑淡入/逐字打字) · 停留 (1-2s) · 出场 (缓淡出/下滑). Key subtitles (hook/data/CTA) get one short SFX, e.g. `"叮",前0.1s背景降6dB反衬`.

**字幕反乱码规则 (anti-mojibake, every Chinese subtitle segment)**: 字体族 must be a CJK-capable, embedded font that ships with the Provider's renderer (e.g. `Noto Sans CJK SC`, `Source Han Sans SC`, `PingFang SC`, `Microsoft YaHei`); never use a pure-Latin font. State it once in the storyboard header and once verbatim inside the constraint block of §6, e.g. `所有中文字幕字体: Noto Sans CJK SC (CJK 全字符集 + UTF-8 编码),禁止使用拉丁字体`. Copy rules: 品牌名/型号/成分名零容错 — verbatim from the user or confirm; ≤12 字 per line, split longer ones; wrap numbers/units/brand names in English quotes so the model keeps them verbatim; avoid absolute claims; 行动号召 carries urgency when the goal is conversion (仅限今日/前100名).
...
Close every prompt with a constraint block: `全片约束: 避免纯色渐变背景,避免棚拍空盒子感; 色调从X自然过渡到Y,过渡流畅不跳戏; 所有中文字幕字体固定 Noto Sans CJK SC,UTF-8 编码,禁止使用拉丁字体/缺字回退,所有中文按引号内文字逐字渲染,零错字零乱码`. For vertical delivery also state `竖屏9:16` explicitly in the prompt header.
...
- When splitting (>22s or Provider cap), every segment satisfies §2-§5 independently. Segment consistency comes from: 美学母体照抄, 后段以前段末帧作首帧 (§6.1), 同一字幕字体族 (e.g. `Noto Sans CJK SC`) 与同一色板贯穿所有分段, one locked subtitle/audio style set across all segments.
