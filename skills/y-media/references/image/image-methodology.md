...
### 0.5 Reference 路由(按用途选最匹配的)

> 当任务属于以下类型时,优先读对应 reference 获取领域特定指导。Reference 是"某次任务下的样例",看写法逻辑,不要照搬。
...
> **何时不读 Reference**:通用产品图/人像/氛围图无特定领域模板时,直接用本方法论 + [image-example.md](image-example.md)。
...
## 4.5 媒介统一原则(Medium Unity)

> 从 visual-image-generator 吸收的核心经验。整张图应该是"一件用统一媒介做出来的设计作品",而不是"一张照片 + 后期盖上去的文字"。文字打印感、贴图感的根因就是"照片"和"字库字"是两种料,天然贴不到一起。

**执行规则**:

1. **优先让画面走媒介感强的路线**:水彩插画 / 版画 / 做旧印刷海报 / 水墨 / 蜡笔 / 拼贴 / 丝网印刷。写实感来自具体名词:具体地点、具体材质、具体光的时间。
2. **主标和文字用同一种媒介"画"出来**:水墨图配书法飞白、水彩插画配手写蜡笔字、做旧印刷配撕纸做旧字、霓虹灯配夜景发光字。
3. **确需用实拍照片时**:必须让文字和场景发生关系——叠压主体、承接画面光影、贴合材质,而不是老实待在空白区。

**自检**:把文字层单独抠出来看,它像不像从这张图的材质里"长"出来的?如果像贴上去的标签,就是媒介没统一。

## 4.6 风格总纲构造法(Style Direction)

> 七维度骨架的"风格总领句"不能是抽象形容词("高级感""复古""简约"),必须是一个**自造的短词组**,模型会自己推出版式、装饰、氛围。

**构造法**:`题材气质 + 表达手法 + 编辑属性`

| 题材气质 | 表达手法 | 编辑属性 | 组合示例 |
| --- | --- | --- | --- |
| 深夜巷道摄影 | 霓虹 | 编辑设计 | 深夜巷道摄影结合霓虹编辑设计 |
| 浮世绘线条 | 现代排版 | 拼贴 | 浮世绘线条与现代排版拼贴 |
| 潮湿雨林纪实 | 博物馆图鉴 | 排版 | 潮湿雨林纪实结合博物馆图鉴排版 |
| 独立杂志剪贴 | 做旧印刷 | 日记感 | 独立杂志剪贴结合做旧印刷日记感 |

**铁律**:不管题材新旧、也不管是否似曾相识,**每张图都当场重新组一个词组**——同题材下一张也不例外。Reference 里出现的所有词组都是"某次任务下的样例",看写法逻辑,不要照搬到下一次。

## 4.7 文字融入画面(Text Integration)

> 文字不是事后补的装饰层,而是画面图形的一部分。让文字本身也成为图形——挑一两个手法用上:

| 手法 | 适用 | 示例 |
| --- | --- | --- |
| 超大艺术化数字/日期 | 海报/封面 | "2026" 占画面 1/3,带材质 |
| 重复文字铺成背景水印肌理 | 品牌/氛围 | 品牌名重复铺满背景,低透明度 |
| 竖排文字 | 国风/文艺 | 主标题竖排,配印章 |
| 做旧描边英文块 | 编辑/拼贴 | 撕纸质感英文字母块 |
| 印章徽章角标 | 品牌/认证 | 圆形印章,做旧边缘 |
| 主体与文字穿插叠压 | 海报/KV | 主标题被主体遮挡一部分,形成前后关系 |

**信息图 / 电商促销 / 内容封面要密就密,不设上限**。

## 4.8 安全边距(Safe Margin)

> 生图模型默认会把大标题顶到画面最上沿、把主体裁掉一半。所以必须在 prompt 里正向要求留白。

**硬约束**(每条都要正向写进 prompt):

1. **四周留安全边距**:prompt 结尾固定加一句 `主标题、主体和所有文字完整居中在画框内,四周留出安全边距,不贴边、不裁切`。这是正向要求留白,不是负向排除。
2. **主标是"带材质、和画面同源"的艺术字**:中文字形不吃"编辑设计""科技编辑"这类抽象风格词,不给具体形态就 100% 回落系统黑体/宋体、变成打印感。所以文字段里给主标点明一种**带肌理、和画面同一种料**的艺术字:
...
— 主标题、主体和所有文字完整居中在画框内,四周留出安全边距,不贴边、不裁切(§4.8)
— 主标题使用<指定艺术字形态>,与画面同一种媒介质感,不是直接把字打在图上(§4.8)
— 每组文字都要有融入画面的视觉设计,和画面同一种媒介质感(§4.7)
```
...
以一句话锁定风格类型 + 画幅 + 美学基调。**风格总领句必须是自造短词组**(§4.6),不能是抽象形容词。然后展开七维度:
...
| 维度 | 规则 | 示例 |
| --- | --- | --- |
| 风格总领句 | **自造短词组**:`题材气质 + 表达手法 + 编辑属性`(§4.6);一行内给出风格类型 + 画幅比例 + 美学基调 | 独立杂志剪贴结合做旧印刷日记感,vertical 3:4, warm afternoon zine aesthetic |
...
| 约束 | 防 AI 合成感 / 文字 / 负向 / **安全边距** / **艺术字形态** | 避免纯色渐变背景;主标题、主体和所有文字完整居中在画框内,四周留出安全边距;主标题使用<指定艺术字形态> |

**风格总领句自检**:这个词组是不是"自造"的?如果直接用了"高级感""复古""简约"这种抽象词,或者照搬了 reference 里的词组,就重新构造一个。
