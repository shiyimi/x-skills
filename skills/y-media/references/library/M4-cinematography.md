# M4. cinematography · 镜头语言(景别/机位/运镜/构图)

> 摄影指导的职责：把"拍什么"落成"怎么拍"。参考真实电影的镜头语言，落到 AI 视频 prompt 可直接套用的速查。
>
> **源头写对**:镜头语言是规划阶段就决定的，写进 `Final Prompt` 的 `★ Camera language` 段，不做后期补救。
>
> **与 [M1-methodology.md](M1-methodology.md) 关系**:M1 §5 管"镜头结构怎么写(1:1 镜号/时间锚点)"，本文件管"每个镜头用什么镜头语言(景别/机位/运镜/构图)"。M1 §7 分镜表的"景别/视角"与"运镜"两列，本文件是这两列的取值词典。
...
| **L3** | **数字分级写 prompt** | 技术参数(焦段/色温 K/角度)→ 仅展示层，执行层语义化；**剧情爆点数字(金额/名称/年龄)→ 必须写进 prompt**(模型需直出，如"9000""八千") |
...
## §7 实例提炼(从 examples 高频镜头)

> 从 5 个 example 的 `★ Camera language` 段提炼的真实用法，可直接照抄结构。
...
- **逐镜 1:1**:`★ Camera language` 段按分镜表镜号 1:1 展开，每镜一个时间锚点(见 [M1 §5](M1-methodology.md))。
- **≤2 运镜**:单段组合运镜 ≤2 个(L1)。
