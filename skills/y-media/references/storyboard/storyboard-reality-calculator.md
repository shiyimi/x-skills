...
| 总时长 >18s | ✗ 必拆 | ✓ keyframes-to-video 接续 |
...
输入: 目标时长 T + 镜头数 N
  ...
  │    └─ N > 3 → 必须多段
  │
  └─ T > 18s? → 必拆,首选 keyframes-to-video,首帧=前段末帧
```
...
| 帧数 > 441 | 拆段为多段,首选 keyframes-to-video(Provider 内部接续,不需本地 ffmpeg);若需转场/硬切则在 storyboard.md `Generation` 段附拼接菜谱 |
