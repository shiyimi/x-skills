...
- **S 级（超小任务）**：文案修改、样式微调、局部明显 Bug 修复；不得产出测试用例。
- **M 级（中等任务）**：常规功能开发、模块改造、接口联调；不得产出测试用例。
- **L 级（复杂任务）**：跨模块重构、复杂性能优化、架构调整；仅在必要时产出测试用例，且必须说明必要性。

S 级允许精简输出；M/L 级应完整执行 Step 2（深度可按复杂度调整）。
“必要时”指高回归风险、核心分支复杂逻辑、跨模块影响面大，或用户明确要求验证方案。
...
- 必要处添加中文注释（说明 Why）
- 保证基本 A11y
- 测试用例策略：M 级不得产出测试用例；L 级仅在必要时产出，并说明触发依据
...
3. **测试约束检查**：S/M 是否未产出测试用例；L 级若产出测试用例，是否说明必要性；未触发必要条件时，是否明确“不产出测试用例”。
4. **可运行性检查**：语法/类型是否可过；依赖、环境变量、启动命令是否明确。
5. **执行真实性检查**：明确“未执行项”、原因与残余风险。
6. **自我修正**：披露实现中发现的问题与修复。
