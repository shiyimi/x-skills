...
Require Node.js 18 or newer and locate `skills/y-media/core/media.cjs` relative to this file. Read [skills/y-media/core/provider-contract.md](skills/y-media/core/provider-contract.md) for request/result shapes and errors. Read a Provider's colocated reference, such as [skills/y-media/providers/agnes/api.md](skills/y-media/providers/agnes/api.md), only for its credentials, models, and parameters.
...
| --- | --- | --- | --- |
| 1. 路径决策 | §1.2 (G/E/H) | [skills/y-media/references/image/image-methodology.md](skills/y-media/references/image/image-methodology.md) §1.2 (G/E/H) | 概念相同,image 三选一 |
| 2. 场景模板 | [skills/y-media/references/video/scenes/scene-commerce-product.md](skills/y-media/references/video/scenes/scene-commerce-product.md) 6 大品类 | 共享同一模板,取"产品摄影"相关列 | image 与 video 同源 |
...
| 6. 合规清单 | [skills/y-media/references/video/scenes/scene-commerce-product.md](skills/y-media/references/video/scenes/scene-commerce-product.md) §9 | 同 §9(广告法/品牌/价格/资质/版权 BGM/母婴伦理;image 路径跳过"版权 BGM"那行) | 共享清单 |
...
**Multi-segment flow (dynamic cap)**: the single-segment cap is the selected Provider's `capability_limits[<capability>].maxSingleSegmentDuration` (read via `capabilities`, not hardcoded). When the target exceeds it, **stop at the §2 Collect gate and ask the user** to choose between ① split (deliver N segments as separate files, no recipe) and ② merge (deliver N segments + external merge recipe in the sidecar). Do not auto-decide, do not auto-merge, and do not hardcode `18s/441`. See [skills/y-media/references/video/storyboard-methodology.md](skills/y-media/references/video/storyboard-methodology.md) §7.1 for the confirmation template.
...
The Skill owns the creative layer: turn the collected brief into a concrete storyboard document before any submission. Core never plans or generates storyboards. Follow [skills/y-media/references/video/storyboard-methodology.md](skills/y-media/references/video/storyboard-methodology.md) for every creative decision below.

For images, produce one final prompt per §6 of [skills/y-media/references/image/image-methodology.md](skills/y-media/references/image/image-methodology.md) and select `text-to-image` or `image-to-image` (or `H` multi-zone for layered layouts). Review the complete example in [skills/y-media/references/image/image-example.md](skills/y-media/references/image/image-example.md) before writing an image prompt with layered layout or typography — it is the granularity calibration anchor for the `<name>.image-brief.md` sidecar. For `image-to-image`, state explicitly what to preserve and what to change (element-level binding). Pick the aspect ratio by intended use and put `size`/`ratio` in `parameters`. The image sidecar is `<output filename stem>.image-brief.md` (parallel to `<name>.video-brief.md` for video).

For videos, generate a storyboard document per §1-§3 and §6 of the methodology and save it next to the intended output as `<output filename stem>.video-brief.md`. Review the complete example in [skills/y-media/references/video/storyboard-example.md](skills/y-media/references/video/storyboard-example.md) before writing the document — it is the granularity calibration anchor; rules without a filled example produce vague output. The document carries the brief, the shot table, and the full video prompt, and it doubles as the delivery sidecar: Step 7 appends `Generation` metadata instead of writing a second file.
...
- [skills/y-media/references/cinematography-reference.md](skills/y-media/references/cinematography-reference.md) — 影视要素词典:景别/运镜/光影/色彩/声音/剪辑/构图/焦段的具体术语与数值. Consult when filling any shot-table column to pick the exact term.
- [skills/y-media/references/influence-factors.md](skills/y-media/references/influence-factors.md) — 视频生成影响因子 F1-F12(带权重评分卡):每个因子的有效填写阈值与失控修复. Consult to decide how具体 a field must be and to fix generation failures.
- [skills/y-media/references/video/t2v-model-capability.md](skills/y-media/references/video/t2v-model-capability.md) — t2v 模型能力边界 M1-M6 + 展示层vs执行层对照表 + 时间表达方式. **Read before writing any prompt** — decides which parameters enter the prompt (semantic) vs which leave (numeric/audio).
- [skills/y-media/references/video/prompt-structure-formula.md](skills/y-media/references/video/prompt-structure-formula.md) — prompt 写作骨架:八要素+五定法+角色四层+场景三层+14镜头库+避坑三陷阱+5铁律. Consult when assembling the prompt block.
- [skills/y-media/references/video/cinematic-shot-library.md](skills/y-media/references/video/cinematic-shot-library.md) — 14 镜头库(6 运镜组合+4 高级术语+4 构图技法)+ 镜头选择决策表. Consult when picking camera moves.
- [skills/y-media/references/video/granularity-scale.md](skills/y-media/references/video/granularity-scale.md) — 颗粒度标尺:展示层/执行层分工 + 12 字段抽象→具体对照. Consult when unsure how具体 a column should be.
- [skills/y-media/references/video/pitfalls-and-iron-rules.md](skills/y-media/references/video/pitfalls-and-iron-rules.md) — 避坑三陷阱(物理互斥/静止动词/光影缺失)+ 5 铁律. Run as final self-check before submission.
- [skills/y-media/references/video/negative-prompt-methodology.md](skills/y-media/references/video/negative-prompt-methodology.md) — 视频路径 `negative_prompt` 字段方法论:与 Hard Constraints 的分工、何时必填、内容来源、提交前自检. **mandatory when work = video and the sidecar has a Negative Prompt section**;image 路径不适用.
- [skills/y-media/references/video/storyboard-reality-calculator.md](skills/y-media/references/video/storyboard-reality-calculator.md) — 镜头现实性决策 + Python 帧数校验 + Provider 能力速查. Run before submission to verify frame count and single-shot vs multi-segment feasibility.
- [skills/y-media/references/video/templates/templates-3-sets.md](skills/y-media/references/video/templates/templates-3-sets.md) + [skills/y-media/references/video/templates/scene-quick-match.md](skills/y-media/references/video/templates/scene-quick-match.md) — 3 套即用模板 + 场景速配表 + 主体描述速查 + 钩子速查. Consult for simple needs or quick scene routing.
- [skills/y-media/references/video/scenes/scene-nature-animal.md](skills/y-media/references/video/scenes/scene-nature-animal.md) / [scene-lifestyle-aesthetic.md](skills/y-media/references/video/scenes/scene-lifestyle-aesthetic.md) / [scene-portrait-fashion.md](skills/y-media/references/video/scenes/scene-portrait-fashion.md) / [scene-food-asmr.md](skills/y-media/references/video/scenes/scene-food-asmr.md) — 4 类场景参考(自然/生活/时尚/美食)的光影/音频/骨架/高潮点模板. Consult when the subject matches a known type.
- [skills/y-media/references/video/scenes/scene-commerce-product.md](skills/y-media/references/video/scenes/scene-commerce-product.md) — **commerce mode 专属** 6 大品类(美妆/服饰/家居/数码/母婴/通用)的产品演示模板;含光影/音频/高潮点/反模式/合规清单/R5 零错字铁律. **mandatory when mode = commerce (§1.1)**;otherwise skip.
- [skills/y-media/references/image/image-methodology.md](skills/y-media/references/image/image-methodology.md) — **image 路径方法论主文件**(平级于 storyboard-methodology.md). 完整 R1-R5 单帧降维 + 路径判定(G/E/H) + 视觉骨架 + 视觉规范表 + 文字版式 + 七维度 prompt + 系列图一致性 + i2v 桥接 + 反模式. **mandatory when work = image**.
- [skills/y-media/references/image/image-example.md](skills/y-media/references/image/image-example.md) — image-brief.md 完整示例(3 套:G 路径产品图 / H 路径海报拼贴 / E 骨架 Logo)+ 反例对照表. Granularity calibration anchor for image sidecars.
...
Default to vertical 9:16 and map duration to Provider frames: `num_frames = round(duration_seconds * frame_rate)`. The hard caps are `capability_limits[<capability>].maxFrames` and `capability_limits[<capability>].maxSingleSegmentDuration` (read via `capabilities`, do not hardcode 18s/441). Both must satisfy the `frameCountRule` (e.g. 8n+1). **Do not hardcode 18s/441** in any prompt or storyboard — different Providers have different caps. For durations that exceed the cap, plan N segments, submit each independently, then ask the user about merging. See [skills/y-media/references/video/storyboard-methodology.md](skills/y-media/references/video/storyboard-methodology.md) §7. Express aspect ratio through `parameters.width/height`.
...
Build the public request defined in [skills/y-media/core/provider-contract.md](skills/y-media/core/provider-contract.md). Keep Provider-specific controls in `parameters` and verify them against the selected Provider reference.
